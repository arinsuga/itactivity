...
margin-left: auto; margin-right:auto;">
    <div class="card-body">
      ...
      <div class="form-group">
        ...
        @if ($fieldEnabled == true)
          <select onchange="loadProductsubtype();" name="producttype_id" class="form-control">
      ...
      <!-- text input:text -->
      <div class="form-group">
        <label>Sub Tipe Produk</label>
        @if ($fieldEnabled == true)
          <select name="productsubtype_id" class="form-control">
                @foreach ($productsubtype as $key => $item)
                <option value="{{ $item->id }}" {{ ( $viewModel->data->productsubtype_id == $item->id) ? 'selected' : '' }}>{{ $item->name }}</option>
                @endforeach
            </select>
        @else
          <input type="hidden" name="productsubtype_id" value="{{ $viewModel->data->productsubtype_id }}" readonly>
          <div class="form-group">
              <input disabled type="text" value="{{ $viewModel->data->productsubtype != null ? $viewModel->data->productsubtype->name : '' }}" class="form-control">
          </div>
        @endif
        <strong>{{ $errors->first('producttype_id') }}</strong>
